	...
	if (empty($log_user) || is_null($log_user)) {
		...
		if ($userinfo) {
			...
			$cols									= "
				...
				get_call_launch,
				am_message_exten,
				xferconf_a_dtmf AS Call_XC_a_DTMF,
				xferconf_a_number AS Call_XC_a_Number,
				xferconf_b_dtmf AS Call_XC_b_DTMF,
				xferconf_b_number AS Call_XC_b_Number,
				alt_number_dialing,
				scheduled_callbacks,
				wrapup_seconds,
				wrapup_message,
				...
				display_queue_count,
				manual_dial_filter,
				agent_clipboard_copy AS Copy_to_Clipboard,
				use_campaign_dnc,
				three_way_call_cid,
				dial_method,
				...
				start_call_url,
				dispo_call_url,
				xferconf_c_number AS Call_XC_c_Number,
				xferconf_d_number AS Call_XC_d_Number,
				xferconf_e_number AS Call_XC_e_Number,
				use_custom_cid,
			...
			if ($campinfo) {
				...
				if ( ($campinfo['campaign_allow_inbound'] == 'Y') && ($campinfo['dial_method'] != 'MANUAL') ) {
					...
					$rslt 							= $astDB
						->where('active', 'Y')
						->where('group_id', $closer_campaigns, 'IN')
